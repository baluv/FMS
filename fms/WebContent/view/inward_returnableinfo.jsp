			...
			<tbody>
				<c:forEach items="${InwardList}" var="item">
					<tr style="cursor: pointer" onmouseover="this.background='#C0C0C0'"">
						...
						<td><a href="updateinward.html?id=${item.id}&date=${item.date}&invoicenumber=${item.invoicenumber}&fromwhere=${item.fromwhere}&towhere=${item.towhere}&edate=${item.edate}&rdate=${item.rdate}&takenby=${item.takenby}">Edit</a></td>
						<td><a href="inwardreturnablequantity.html?id=${item.id}">View Quantity</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
