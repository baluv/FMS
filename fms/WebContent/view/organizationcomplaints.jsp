        ...
        <div id="container">
            <div id="demo_jui">
		        <table id="complaints" class="display">
		            ...
		            <tbody>
		          		 <c:forEach items="${orgcomplaintList}" var="item">
						  <tr style="cursor:pointer" onmouseover="this.background='#C0C0C0'" >
						    ...
						    <td><a href="orgupdateComplaint.html?complaintId=${item.complaintId}&complaint=${item.complaint}&roomNumber=${item.roomNumber}&priority=${item.priority}&category=${item.category}&machine=${item.machine}&assignedto=${item.assignedto}&status=${item.status}&comments=${item.comments} ">Update</a></td>
						    <td><a href="orgvoucher.html?cid=${item.complaintId}">Voucher</a></td>
						 </tr>
						 </c:forEach>
		            </tbody>       
		        </table>
