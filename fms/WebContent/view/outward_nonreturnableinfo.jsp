			...
			<tbody>
				<c:forEach items="${OutwardList}" var="item">
					<tr style="cursor: pointer" onmouseover="this.background='#C0C0C0'"">
						...
						<td><a href="updateoutwardnonreturnable.html?id=${item.id}&date=${item.date}&gatepass=${item.gatepass}&fromwhere=${item.fromwhere}&towhere=${item.towhere}">Edit</a></td>
						
						<td><a href="outwardnonreturnablequantity.html?id=${item.id}">View Quantity</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
