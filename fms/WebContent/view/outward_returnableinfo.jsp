...
<div id="page2">
	...
	<c:if test="${!empty OutwardList}">

		<div id="dt_example">
		<div id="container">
		<div id="demo_jui">
		<table id="Outwardinfo" class="display">
			<thead>
				<tr>
					<th align="left">Id</th>
					<th align="left">Date</th>
					<th align="left">Gate Pass</th>
			...
			<tbody>
				<c:forEach items="${OutwardList}" var="item">
					<tr style="cursor: pointer" onmouseover="this.background='#C0C0C0'"">
						...
						<td><c:out value="${item.authorizedby}"></c:out></td>
						<td><a href="updateoutward.html?id=${item.id}&date=${item.date}&gatepass=${item.gatepass}&fromwhere=${item.fromwhere}&towhere=${item.towhere}&edate=${item.edate}&rdate=${item.rdate}&authorizedby=${item.authorizedby}">Edit</a></td>
						<td><a href="outwardreturnablequantity.html?id=${item.id}">View Quantity</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
