							...
							<c:forEach items="${assetList}" var="item">
								<tbody>
									<tr>
										...
										<td><a href="editroomassetform.html?id=${item.id}&itemName=${item.itemName}&itemCount=${item.itemCount}">Edit</a></td>
										<td><a href="deleteroomasset.html?id=${item.id}"  onclick="return confirm('Are you sure you want to delete?')">Delete</a></td>
									</tr>
								</tbody>
							</c:forEach>
					<tfoot>
